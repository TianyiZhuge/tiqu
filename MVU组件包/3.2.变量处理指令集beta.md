...
**Beta 版特性**：
- 引入 `_.add`、`_.assign`、`_.remove` 命令，实现更精细的变量操作
...
- **命令多样性**：示例中应展示多种命令的使用（`_.set`、`_.add`、`_.assign`、`_.remove`）
...
```javascript
// 数组：添加元素
_.assign('array_path', 'new_item');

// 对象：添加或修改键值对
_.assign('object_path', 'key', 'value');
```

**示例**：
```javascript
// 向数组添加元素
_.assign('背包.物品列表', '诗织的手帕');

// 向对象添加键值对
_.assign('记忆', '收到诗织的赠礼', '从诗织那里收到了手帕');

// 向对象添加嵌套对象
_.assign('角色.技能', '火球术', {等级: 1, 熟练度: 0});
```
