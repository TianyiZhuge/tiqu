...
- 在 LLM 执行 `_.assign` / `_.remove` 等结构性变更时进行校验
- 阻止未授权的结构变动
...
- **`extensible`**: `boolean`，默认 `false`
  ...
  - `true`：对象结构开放，允许 `_.assign` 新增键、`_.remove` 删除键
...
"与《3.2.变量处理指令集beta.md》配合使用，以启用 `_.add`、`_.assign`、`_.remove` 等增强命令和模式校验。与《2.变量更新规则.md》保持一致，初始化与更新逻辑分离。"
