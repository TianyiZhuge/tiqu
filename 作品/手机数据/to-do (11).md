...
1. **手机数据.联系人.姓名.昵称**：以角色名为key，存储数据，昵称，字符串
2. **手机数据.联系人.姓名.签名**：以角色名为key，存储数据，签名，字符串
3. **_[待填写]_**：_[待填写]_
...
在项目根目录创建一个 YAML 配置文件（例如：`手机数据.yaml`），内容参考以下格式：
...
# 主要字段映射
fields:
  description: 作品/手机数据/角色设定_主角名.xyaml
  personality: ""
  scenario: ""
  first_mes: 作品/手机数据/开场白.md
  mes_example: ""
  creator_notes: ""
  system_prompt: ""
...
extensions:
  ...
  fav: false
  world: 手机数据
  status_bar: 作品/手机数据/状态栏.html  # 可选
...
character_book:
  ...
  entries:
    # [InitVar]初始化条目
    - comment: "[InitVar]初始化"
      content: 作品/手机数据/变量初始化_beta.xyaml
      enabled: false
    ...
    # 变量处理指令集
    - comment: "变量处理指令集"
      content: 作品/手机数据/变量处理指令集_beta.xyaml
      enabled: true
      position: at_depth
      insertion_order: 2
      depth: 1
    ...
    # 玩家角色
    - comment: "玩家角色_{{user}}"
      content: 作品/手机数据/玩家角色_{{user}}.xyaml
      enabled: true
      position: before_char
      insertion_order: 2
      depth: 4
...
# 运行打包程序
node build-card.js 手机数据.yaml
```

程序会自动读取配置文件，整合所有内容，生成 `手机数据.json` 文件。
